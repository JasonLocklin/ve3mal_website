...
while(<>) {
	...
	my $file = $ARGV; $file =~ s/.*\///;
	my $newfile = $file; $newfile =~ s/^\d._//s;
	...
	if ($$curref{'url'}) {
		if ($$curref{'url'} eq 'FILE') {
			$$curref{"dis:$newfile"} = 1;
		}
		if ($$curref{'url'} =~ s/FILE/$newfile/ ) {
...
for my $file (@argv) {
	...
	$file =~ s/.*\///;
	my $outfile = $file; $outfile =~ s/^\d._//s;
	$outfile =~ s/.*\///;
	open(STDOUT, "> $outputdir/$outfile")
		or die("Cannot open output file $outputdir/$outfile: $!\n");
	...
	close(STDOUT) || die("Cannot close output file $outputdir/$outfile: $!\n");
	close(IN);
}
